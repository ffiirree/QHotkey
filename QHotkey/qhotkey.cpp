...
void QHotkey::addGlobalMapping(const QKeySequence& shortcut, QHotkey::NativeShortcut nativeShortcut)
{
    ...
    QMetaObject::invokeMethod(
        ...
        Q_ARG(Qt::Key, static_cast<Qt::Key>(key & ~Qt::KeyboardModifierMask)),
        Q_ARG(Qt::KeyboardModifiers, Qt::KeyboardModifiers(key & Qt::KeyboardModifierMask)),
        Q_ARG(QHotkey::NativeShortcut, nativeShortcut));
}

bool QHotkey::isPlatformSupported() { return QHotkeyPrivate::isPlatformSupported(); }

QHotkey::QHotkey(QObject *parent)
    : QObject(parent), _keyCode(Qt::Key_unknown), _modifiers(Qt::NoModifier), _registered(false)
{}

QHotkey::QHotkey(const QKeySequence& shortcut, bool autoRegister, QObject *parent) : QHotkey(parent)
{
...
QHotkey::QHotkey(NativeShortcut shortcut, bool autoRegister, QObject *parent) : QHotkey(parent)
{
...
{
    if (_keyCode == Qt::Key_unknown) return {};

#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    return QKeySequence((_keyCode | _modifiers).toCombined());
#else
    return { static_cast<int>(_keyCode | _modifiers) };
#endif
}
...
QHotkey::NativeShortcut QHotkey::currentNativeShortcut() const { return _nativeShortcut; }

bool QHotkey::isRegistered() const { return _registered; }

bool QHotkey::setShortcut(const QKeySequence& shortcut, bool autoRegister)
{
    if (shortcut.isEmpty()) return resetShortcut();
    if (shortcut.count() > 1) {
    ...
    return setShortcut(static_cast<Qt::Key>(key & ~Qt::KeyboardModifierMask),
                       Qt::KeyboardModifiers(key & Qt::KeyboardModifierMask), autoRegister);
}
...
bool QHotkey::setShortcut(Qt::Key keyCode, Qt::KeyboardModifiers modifiers, bool autoRegister)
{
    if (_registered) {
        if (autoRegister) {
            if (!QHotkeyPrivate::instance()->removeShortcut(this)) return false;
        }
        else
            return false;
    }
    ...
    if (_nativeShortcut.isValid()) {
        if (autoRegister) return QHotkeyPrivate::instance()->addShortcut(this);
        return true;
    }

    qCWarning(logQHotkey) << "Unable to map shortcut to native keys. Key:" << keyCode
                          << "Modifiers:" << modifiers;
    _keyCode        = Qt::Key_unknown;
    _modifiers      = Qt::NoModifier;
    _nativeShortcut = NativeShortcut();
...
bool QHotkey::setNativeShortcut(NativeShortcut nativeShortcut, bool autoRegister)
{
    if (_registered) {
        if (autoRegister) {
            if (!QHotkeyPrivate::instance()->removeShortcut(this)) return false;
...
{
    if (_registered && !registered) return QHotkeyPrivate::instance()->removeShortcut(this);
    if (!_registered && registered) {
        if (!_nativeShortcut.isValid()) return false;
...
QHotkeyPrivate::~QHotkeyPrivate()
{
    ...
    if (qApp && qApp->eventDispatcher()) qApp->eventDispatcher()->removeNativeEventFilter(this);
}

QHotkey::NativeShortcut QHotkeyPrivate::nativeShortcut(Qt::Key keycode, Qt::KeyboardModifiers modifiers)
{
    const Qt::ConnectionType conType =
    ...
    if (!QMetaObject::invokeMethod(this, "nativeShortcutInvoked", conType,
                                   Q_RETURN_ARG(QHotkey::NativeShortcut, res), Q_ARG(Qt::Key, keycode),
...
{
    if (hotkey->_registered) return false;

    const Qt::ConnectionType conType =
        QThread::currentThread() == thread() ? Qt::DirectConnection : Qt::BlockingQueuedConnection;
    bool res = false;
    if (!QMetaObject::invokeMethod(this, "addShortcutInvoked", conType, Q_RETURN_ARG(bool, res),
                                   Q_ARG(QHotkey *, hotkey))) {
        return false;
    }

    if (res) emit hotkey->registeredChanged(true);
    return res;
}
...
bool QHotkeyPrivate::removeShortcut(QHotkey *hotkey)
{
    ...
    const Qt::ConnectionType conType =
        QThread::currentThread() == thread() ? Qt::DirectConnection : Qt::BlockingQueuedConnection;
    bool res = false;
    if (!QMetaObject::invokeMethod(this, "removeShortcutInvoked", conType, Q_RETURN_ARG(bool, res),
                                   Q_ARG(QHotkey *, hotkey))) {
        return false;
    }

    if (res) emit hotkey->registeredChanged(false);
    return res;
}

void QHotkeyPrivate::activateShortcut(QHotkey::NativeShortcut shortcut)
{
    const QMetaMethod signal = QMetaMethod::fromSignal(&QHotkey::activated);
    for (QHotkey *hkey : shortcuts.values(shortcut)) signal.invoke(hkey, Qt::QueuedConnection);
}

void QHotkeyPrivate::releaseShortcut(QHotkey::NativeShortcut shortcut)
{
    const QMetaMethod signal = QMetaMethod::fromSignal(&QHotkey::released);
    for (QHotkey *hkey : shortcuts.values(shortcut)) signal.invoke(hkey, Qt::QueuedConnection);
}
...
{
    const auto shortcut = hotkey->_nativeShortcut;
...
bool QHotkeyPrivate::removeShortcutInvoked(QHotkey *hotkey)
{
    ...
    if (shortcuts.remove(shortcut, hotkey) == 0) return false;
...
QHotkey::NativeShortcut QHotkeyPrivate::nativeShortcutInvoked(Qt::Key               keycode,
                                                              Qt::KeyboardModifiers modifiers)
{
    if (mapping.contains({ keycode, modifiers })) return mapping.value({ keycode, modifiers });

    bool ok1 = false;
    auto k   = nativeKeycode(keycode, ok1);
    bool ok2 = false;
    auto m   = nativeModifiers(modifiers, ok2);
    if (ok1 && ok2) return { k, m };
    return {};
}
...
bool QHotkey::NativeShortcut::operator==(const NativeShortcut& other) const
{
...
QHOTKEY_HASH_SEED qHash(QHotkey::NativeShortcut key) { return qHash(key.key) ^ qHash(key.modifier); }
